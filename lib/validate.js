...
// Deterministic lint of .specs/: citation file/line existence, unique IDs,
// cross-doc links, index.json sync. The semantic layer (EXACT vs HALLUCINATION)
// lives in the scan skill's fidelity judge — this is the cheap gate.
...
export function* sections(text) {
  // yields {id, title, body} for each '### <ID> ...' section
  let current = null;
...
export async function runValidate(root = ".", { writeIndex = false } = {}) {
  root = path.resolve(root);
  const specsDir = path.join(root, ".specs");
  if (!fs.existsSync(specsDir)) {
    console.error("specmine: no .specs/ here — run `npx specmine init` first.");
    process.exitCode = 1;
    return { code: 1, errors: ["no .specs"], warnings: [] };
  }
  ...
  if (writeIndex) {
    ...
    const fileIndex = Object.fromEntries(
      [...rebuilt.entries()].sort().map(([f, ids]) => [f, [...ids].sort()])
    );
    fs.writeFileSync(
      indexFile,
      JSON.stringify(
  ...
  if (!code) console.log("PASS");
  process.exitCode = code;
  return { code, errors, warnings, stats };
}
