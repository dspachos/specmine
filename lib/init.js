...
export async function runInit(root = ".") {
  ...
  // 3. GitHub Action gate (skip if a specmine workflow already exists)
  const workflow = path.join(root, ".github", "workflows", "specmine.yml");
  if (fs.existsSync(workflow)) {
    console.log("Workflow .github/workflows/specmine.yml: already present.");
  } else {
    fs.mkdirSync(path.dirname(workflow), { recursive: true });
    fs.copyFileSync(tplDir("gha", "specmine.yml"), workflow);
    console.log("Workflow installed: .github/workflows/specmine.yml");
  }

  // 4. Pointer block in AGENTS.md if present
